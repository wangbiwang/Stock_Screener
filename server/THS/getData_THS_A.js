    ...
    for (var i = 0; i < datasss.length; i++) {
        ...
        ;(function (time, date) {
            setTimeout(async function () {
                ...
                fs.writeFile(main_files + '.json', JSON.stringify(d), (err) => err)
...
async function getCode(date) {
    let date_n=4
    // ${data_text(date)}
    const data_text = (date) =>
        String(date).substring(0, 4) + '年' + Number(String(date).substring(4, 6)) + '月' + Number(String(date).substring(6, 8)) + '日'
    let c_data=[];
    let res = await axios({
        method: 'post',
        headers: {
        ...
        data:{
            "question": `${data_text(date)}前${date_n}个交易日连续每日收盘价低于开盘价${data_text(date)}涨幅${data_text(date)}9:50的行情收盘价>${data_text(date)}9:50的均线；${data_text(date)}9:50的主力资金流向正、资金流向正、dde大单正、涨幅大于0小于2；${data_text(date)}热度排名小于2000非ST和科创`,
            "perpage": "100",
            "page": 1,
    ...
    if (res.data.status_msg && res.data.status_msg === 'ok') {
         c_data = res.data.data.answer[0].txt[0].content.components[0].data.datas
    } else {
    ...
    // console.log('========',c_data,c_data2,'========')
    // console.log('========',`data1_length:${c_data.length}`,`data2_length:${c_data2.length}`,'========')
    // console.log('========',arr,arr.length,'========')
    console.log('========',`前${date_n}日跌`,date,c_data.length,'========')
    return c_data
