...
;(async function () {
    ...
    const run = async (buyTime = '14:30', sellTime) => {
        let kkt = 0,zzt=0,sydzz=0,syd=0,sylz=new Map()
        ...
        let t_sy = column_bodys[column_bodys.length - 1][column_bodys[column_bodys.length - 1].length - 1]
        let t_gm = column_bodys[column_bodys.length - 1][column_bodys[column_bodys.length - 1].length - 4].map((e) => [
            e.name,
            e.code,
        ])
        return {
